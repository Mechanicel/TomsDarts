...
**Setup-Teilnehmer: Reorder-Button-Feedback stabilisieren (kosmetischer UI-Bug)** —
Beim Umsortieren (↑/↓) der Teilnehmer landete die Touch-Ripple / Highlight nach dem
Swap auf dem deaktivierten (ausgegrauten) Rand-Button. **Ursache:** Die Zeilen waren
per `key(playerId)` an die Spieler gebunden (spieler-stabile Identität); beim Swap
...
- _Phase 3 / „Spiel-Setup-Screen: Anzahl Legs/Sets („Best of X")" abgehakt:_ Setup-Screen um zwei neue Sections erweitert: **`BestOfSection` für Legs + Sets** (je Auswahl aus 1/3/5, nur ungerade). Generische Komponenten `BestOfSection`/`BestOfCard` (an `StartScoreSection`/`StartScoreCard` gespiegelt). **Architektur-Kern:** Einzige Umrechnungsstelle `bestOfToWin(bestOf: Int): Int = (bestOf + 1) / 2` — UI spricht „Best of X", Domäne bleibt „first to N" (`GameConfig.legsToWin`/`setsToWin`, `MatchEngine`). Defaults `DEFAULT_LEGS_BEST_OF=3` (→ legsToWin=2) und `DEFAULT_SETS_BEST_OF=1` (→ setsToWin=1) erhalten bisherige Hartcodierung. Durchreichung über `SetupScreen` → `MainActivity` → `GameScreen` → `GameViewModel.provideFactory` → `GameConfig` + `Match` (persistiert). Neue Strings (Block „Spiel-Setup (Anzahl Legs/Sets)") mit Plurals für Accessibility. Tests: `GameViewModelLegsSetsWiringTest` (alle Leg/Set-Kombinationen + Regressionsschutz) + `SetupScreenConstantsTest`-Erweiterung (Invarianten). 343 Tests grün (gesamt, +12 neu). Entscheidungen in [ADR-0018 Erweiterung](decisions/0018-setup-screen-startpunkt.md#anwendung-phase-3--task-3-legssetsauswahl) festgehalten.
...
**Letzte Aufnahme (Darts des letzten Zugs) pro Spieler auf der Scoreboard-Karte anzeigen** — Aus dem
...
T20/T20/20 → Rest 1 → Bust"). Lösung: **jede Spieler-Karte im Scoreboard** zeigt jetzt
...
für stabile Kartenhöhe. **Barrierefreiheit:** neue Helferfunktion `dartSpokenLabel(dart)` (Teil
...
**Bewusste Design-Entscheidung (dokumentiert):** Pro-Spieler-Isolation statt globale Bar:
Jede Karte ist zuständig für ihren Spieler; `legWon`/`matchWon`-Snapshots rendern `lastTurnDarts`
...
die Vorschlagstabelle **einmalig beim Init** aus allen physikalisch möglichen 1–3-Dart-Kombinationen
(`buildCheckoutTable`) und cacht sie (`CHECKOUT_TABLE`). Kein manuales Lookup-Verzeichnis — statt
dessen wird jeder Eintrag deterministisch berechnet: Das Ergebnis summiert exakt auf den Rest und
endet garantiert auf einem Doppel. **Konsequenz:** Korrektheit ist konstruktionsbedingt, nicht
testabhängig (jeder Route-Eintrag ist per Aufbau korrekt). Bei mehreren gültigen Routen gleicher
Dart-Länge wird die konventionellste gewählt (Komfort-Comparator: bevorzugte Finish-Doppel wie D-20
vor kleineren, bevorzugte Setup-Darts wie T-20 an erster Stelle). `doubleOut=false` → `null` (Feature
ist auf Double-Out zugeschnitten).
